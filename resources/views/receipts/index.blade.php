...
<main class="h-full overflow-y-auto">
          <div class="container px-6 mx-auto grid">
            ...
            <div class="grid  gap-6 mb-8 xl:grid-cols-4">
            <div class="flex justify-center items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800" onclick="window.location.href = '{{ route('receipts.add') }}';">
              ...
              <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap item-center">
                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    <tr class=" text-center text-gray-700 dark:text-gray-400" onclick="window.location.href = '{{ route('receipts.detail',['id'=>$phieuNhap->id]) }}';">
                      <td class="px-4 py-3 text-sm">
                        {{$phieuNhap->id}}
                      </td>
                      <td class="px-4 py-3">                      
                          <p class="font-semibold">{{$phieuNhap->ngay_nhap}}</p>
                      </td>
                      <td class="px-4 py-3 text-sm">
                        {{$phieuNhap->nha_cung_cap->ten }}
                      </td>
                      <td class="px-4 py-3 text-sm">
                        {{$phieuNhap->tong_tien}}
                      </td>
                      <td class="px-4 py-3 text-sm">
                        {{$phieuNhap->nhan_vien->ten}}
                      </td>
                ...
                <div class="mt-4" style="padding-bottom: 50px;">
                  {{ $dsPhieuNhap->links() }} <!-- Hiển thị thanh phân trang -->
                </div> 
                <tbody>
